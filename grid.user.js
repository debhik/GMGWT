...
;(function () {
  ...
  `
    ...
    setInterval(() => {
      ...
      if (buttons && !buttons.__grid_ran) {
        ...
        toggleButton.innerHTML = `
          ...
          <div>
          <div>Time: <output id="timeText">5</output></div>
          <input id="time" type="range" min="1" max="100" value="5">
          <div> Size of Grid: <output id="ctext">4</output></div>
          <input id="sizeofgrid" type="range" min="1" max="100" value="4">
    ...
    function GridLayout(orderingInput) {
      ...
      // if (pinnedIndex < 0) {
      // }
      let pinnedIndex = ret.findIndex(v => v.__gmgvIsPresentation)
      ...
      document.getElementById("time").oninput = function() {
        timeinterval = Number(document.getElementById("time").value) * 1000 //gets the oninput value
        document.getElementById("timetext").innerHTML = val/1000;
      };
      document.getElementById("sizeofgrid").oninput = function() {
        sizeofgrid=Number(document.getElementById("sizeofgrid").value);
        document.getElementById("ctext").innerHTML= sizeofgrid;
      };
      console.log(timeinterval);
      console.log(sizeofgrid);
      let endss;
      if(Number(starts+sizeofgrid) >= Number(ret.length)){
        endss=Number(ret.length);
        console.log("endss is last");
      }else{
        endss= Number(starts+sizeofgrid);
      }
      console.log(starts+' '+endss+' '+sizeofgrid);
      let newret= ret.slice(starts,endss);
      if(endss==Number(ret.length)){
        starts=0;
      }else{
        starts=endss;
      }
      console.log(newret);
      return new VideoList(newret)
    }
